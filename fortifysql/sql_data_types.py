        ...
        if isinstance(number, str):  # check if given string is valid
            for char in number:
                try:
                    ...
                    raise SQLTypeError(
                        f"Error in converting string to INTEGER, value {char} failed"
                    )
    ...
    def __init__(self, number: Number | str):
        if isinstance(number, str):
            try:
                ...
                raise SQLTypeError(
                    f"Error in converting string to REAL, value failed: {e}"
                )
...
class Blob(SQLDataType):
    ...
    def __init__(
        self, bites: bytes = None, bytes_like: Any = None, encoding: str = ""
    ) -> None:
        if bites is not None and bytes_like is not None:
            raise SQLTypeError(
                f"Blob() cannot have two inputs: {bites} and {bytes_like}"
            )
