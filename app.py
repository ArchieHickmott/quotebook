...
def count_likes(quoteid):
    num_likes = db.query(f"SELECT count(quoteid) FROM likes WHERE quoteid={quoteid}")[
        0
    ][0]
...
def submit():
    ...
    if form.validate_on_submit():
        ...
        if len(quote) > 300 or len(name) > 300 or len(date) > 300:
            ...
            db.query(
                "INSERT INTO quotes (name, year, quote) VALUES (?, ?, ?)",
                (name, date, quote),
            )
...
def index():
    ...
    quote = db.query(f"SELECT name, year, quote FROM quotes ORDER BY RANDOM() LIMIT 1")[
        0
    ]
    best_quote = db.query(
        "SELECT name, year, quote FROM quotes ORDER BY numlikes DESC LIMIT 1"
    )[0]
...
def quotes():
    if not "user" in session:
        results = db.query(
            "SELECT name, year, quote, numlikes FROM quotes ORDER BY numlikes DESC"
        )
        quotes = []
        for quote in results:
            quotes.append(Quote(*quote))
...
def search():
    if request.args.get("search"):
        ...
        return render_template(
            "quotes.html", user=user, quotes=results, id=int(user.id())
        )
...
def login():
    ...
    if form.validate_on_submit() and request.method == "POST":
        ...
        try:
            ...
            db.query(
                f"INSERT INTO log_failed_logins VALUES ({id}, ?, ?, ?)",
                (request.remote_addr, datetime.now(), str(e)),
            )
            form.password.errors.append("Something went wrong, please try again. 😢")
        else:  # SUCCESSFUL LOGIN
            db.query(
                f"INSERT INTO log_logins VALUES ({id}, ?, ?)",
...
def home():
    ...
    quote = db.query(
        f"SELECT quoteid, name, year, quote FROM quotes ORDER BY RANDOM() LIMIT 1"
    )[0]
    best_quote = (
        db.quotes.get("quoteid", "name", "year", "quote")
        .order("numlikes DESC")
        .limit(1)[0]
    )
    ...
    best_quote = Quote(*best_quote)
    return render_template(
        "home.html", user=user, id=int(user.id()), quote=quote, best_quote=best_quote
    )
